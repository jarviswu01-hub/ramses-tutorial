<?php
// RAMSeS Demo - Interactive Time-Series Anomaly Detection
...
<head>
    ...
    <title>RAMSeS Demo - Time-Series Anomaly Detection</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #333; background: linear-gradient(135deg, #1a1a2e 0%, #16213e 100%); min-height: 100vh; padding: 20px; }
        ...
        .controls { background: #f5f7fa; padding: 20px; border-radius: 15px; margin-bottom: 20px; display: flex; gap: 15px; flex-wrap: wrap; align-items: center; }
        .btn { background: linear-gradient(135deg, #667eea, #764ba2); color: white; border: none; padding: 12px 24px; border-radius: 25px; cursor: pointer; font-weight: bold; transition: transform 0.2s; }
        .btn:hover { transform: scale(1.05); }
        .btn-secondary { background: linear-gradient(135deg, #00d9ff, #00ff88); color: #1a1a2e; }
        select, input { padding: 10px 15px; border-radius: 8px; border: 2px solid #ddd; font-size: 14px; }
        .chart-container { background: #fff; border-radius: 15px; padding: 20px; margin-bottom: 20px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); }
        .chart-title { font-size: 1.2em; font-weight: bold; margin-bottom: 15px; color: #1a1a2e; }
        canvas { width: 100%; height: 300px; }
        .results { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin-top: 20px; }
        .result-card { background: linear-gradient(135deg, #f5f7fa, #fff); padding: 20px; border-radius: 12px; text-align: center; border-left: 4px solid #667eea; }
        .result-card h4 { color: #667eea; margin-bottom: 5px; }
        .result-card .value { font-size: 1.8em; font-weight: bold; color: #1a1a2e; }
        .explanation { background: #e8f5e9; border-left: 4px solid #00ff88; padding: 15px; border-radius: 0 10px 10px 0; margin: 20px 0; }
        .technique-cards { display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 20px; margin: 20px 0; }
        .technique-card { background: white; border-radius: 15px; padding: 20px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); border-top: 4px solid #667eea; }
        .technique-card h3 { color: #667eea; margin-bottom: 10px; }
        .badge { display: inline-block; background: #667eea; color: white; padding: 4px 12px; border-radius: 20px; font-size: 0.8em; margin-right: 5px; }
...
<body>
    <div class="container">
        <header>
            <h1>RAMSeS Demo</h1>
            <p class="subtitle">Interactive Time-Series Anomaly Detection</p>
        </header>
        
        <div class="content">
            <div class="controls">
                <button class="btn" onclick="generateData()">Generate New Data</button>
                <button class="btn btn-secondary" onclick="runDetection()">Run Detection</button>
                <select id="method">
                    <option value="ramses">RAMSeS (Full Framework)</option>
                    <option value="ensemble">Stacking Ensemble Only</option>
                    <option value="adaptive">Adaptive Selection Only</option>
    ...
    <script>
        ...
        function generateData() {
            const n = 100;
            const anomalyRatio = parseInt(document.getElementById('anomalyRatio').value) / 100;
            timeSeriesData = [];
            anomalyIndices = [];
        ...
        function drawResultChart(detected) {
            ...
            const colors = timeSeriesData.map((_, i) => {
                if (anomalyIndices.includes(i) && detected.includes(i)) return '#00ff88'; // True positive
                if (!anomalyIndices.includes(i) && detected.includes(i)) return '#ffa500'; // False positive
                if (anomalyIndices.includes(i) && !detected.includes(i)) return '#ff6b6b'; // False negative
                return '#667eea'; // Normal
            });
                        ...
                        tooltip: {
                            callbacks: {
                                label: function(ctx) {
                                    ...
                                    if (anomalyIndices.includes(idx) && detected.includes(idx)) return '✓ True Positive';
                                    if (!anomalyIndices.includes(idx) && detected.includes(idx)) return '⚠ False Positive';
                                    if (anomalyIndices.includes(idx) && !detected.includes(idx)) return '✗ Missed';
                                    return 'Normal';
                                }
                            }
                        }
